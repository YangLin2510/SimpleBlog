...
<head>
 ...
 <link rel="stylesheet" href="https://cdn.bootcss.com/bootstrap/4.0.0/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
  <style>
     .container-fluid {background-color: #f5f6f7;min-height: 1000px;}
     #cate {
                ...
                <#list blogsPage.getList() as blog>
                 <div class="bolg">
                    <a href='/blog?articalId=${blog.id}'>
                     <h5>${blog.title}</h5>
                     <p>${blog.blogText}...</p>
                     <div class="bolg-infos"><span class="bolg-type">原创</span><span>${blog.createTime?datetime}</span> <span style="float:right">阅读: ${blog.viewCount}</span> <span style="float:right">评论: ${blog.commentCount}</span></div>
                    </a>
                 </div>
                 </#list>
                    ...
                    <nav>
                       <ul class="pagination pagination-sm justify-content-center">
                         <#if blogsPage.isHasPreviousPage()>
                             <li class="page-item"><a href="/viewBolgsWithAuthor?blogAuthorId=${author.id}&startPage=${blogsPage.getPrePage()}" class="page-link">上一页</a></li>
                         </#if>
                          <#list blogsPage.getNavigatepageNums() as pageNum>
                             <#if blogsPage.getPageNum() == pageNum>
                                    <li class="page-item active" ><a href="/viewBolgsWithAuthor?blogAuthorId=${author.id}&startPage=${pageNum}" class="page-link">${pageNum}</a></li>
                                    <#else>
                                    <li class="page-item"><a href="/viewBolgsWithAuthor?blogAuthorId=${author.id}&startPage=${pageNum}" class="page-link">${pageNum}</a></li>
                             </#if>
                          </#list>
                          <#if blogsPage.isHasNextPage()>
                              <li class="page-item"><a href="/viewBolgsWithAuthor?blogAuthorId=${author.id}&startPage=${blogsPage.getNextPage()}" class="page-link">下一页</a></li>
                          </#if>
                       </ul>
                    </nav>
                 </div>
            ...
            <div class="col-md-4" id="user">
                 ...
                 <div id="user-info">
                   ...
                   <div id="user-info-number" class="row">
                      <#if blogsCountInfo??>
                        <div class="col">发布文章<br> <span style="font-weight:bold">${blogsCountInfo.blog_sum!"0"}</span></div>
                        <div class="col">浏览数<br> <span style="font-weight:bold">${blogsCountInfo.view_sum!"0"}</span></div>
                        <div class="col">评论<br> <span style="font-weight:bold">${blogsCountInfo.comment_sum!"0"}</span></div>
                        <#else>
