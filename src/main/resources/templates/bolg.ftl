            ...
            <div class="col-md-7" id="artical">
                <div class="artical-content">
                    <div id="artical-title">
                        <h2>${blog.title}</h2>
                        <div style="font-size:14px"><span>转载</span> <span
                                style="padding-left:18px">${blog.lastUpdateTime?datetime}</span> <span
                                style="float:right">阅读量: ${blog.viewCount}</span></div>
                    </div>
                    <hr/>
            ...
            <div class="col-md-4" id="user">
                ...
                <div id="user-info">
                    ...
                    <div id="user-info-number" class="row">
                        ...
                        <div class="col">评论<br> <span style="font-weight:bold">${blogsCountInfo.comment_sum}</span>
                        </div>
                    </div>
                ...
                <div class="user-artials">
                    ...
                    <ul class="list-group">
                         <#list blogMonthAchive as achive>
                             ...
                             <#else >
                              <li class="list-group-item" style="display: none"><a
                                      href="/viewBolgsWithAuthor?blogAuthorId=${author.id}&month=${achive.month}">${achive.month}</a>
                                  <span style="float:right;" class="small-font">${achive.blog_count}篇</span></li>
                             </#if>
                         </#list>
            ...
            <div class="col-md-7" id="comments">
                ...
                <form id="public-comment" action="/publishComment?blogId=${blog.id}" method="post">
                    <div class="form-group">
                        <textarea placeholder="输入你的评论..." class="form-control" rows="4" id="comment"
                                  name="comment"></textarea>
                    </div>
                    <div>
                        <span>还能输入<i id="commentLimit">200</i>字</span>
                        <button class="btn btn-outline-primary btn-sm" type="submit">发表评论</button>
                        <button class="btn btn-outline-danger btn-sm"  style="margin-left: 16px;" id="cancelComment" onclick="cancelReplay();return false" >取消评论</button>
                    </div>
                </form>
                    ...
                    <div class="comment-content">
                        <div><a href="#">${commentModel.comment.userName}</a>
                            <span>${commentModel.comment.dateTime?datetime}</span> <span style="float:right"><a
                                    href="javascript:void(0)" onclick="replay('${commentModel.comment.id}','${commentModel.comment.userName}')">回复</a> <span>${comments?size - commentModel_index}楼</span></span></div>
...
<script>
    $(function () {
        prevActionUrl = $("#public-comment").attr("action");
        $("#comment").keydown(checkWordNum);
    });
    ...
    function replay(id,username) {
        $("#comment").val("@"+username+": ");
        var update = prevActionUrl +"&parantCommentId="+id;
        $("#public-comment").attr('action',update);
        $("#comment").focus();
        checkWordNum();
    }
    ...
    function cancelReplay() {
        $("#comment").val("");
        $("#public-comment").attr('action',prevActionUrl);
        $("#commentLimit").html(200);
    }


    function displayFullAchive(obj) {
        $('.user-artials li').css("display", "");
        $(obj).parent().css("display", "none");//去掉展开按钮
        return false;
    }
